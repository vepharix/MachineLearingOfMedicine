...
第四，当前只处理了明确的 `-1` 缺失和重复记录，没有完成逐变量、经医学审核的异常值规则。第五，SAPS-I/SOFA 参照不是严格时间匹配，不能作为完全公平的早期评分对照。第六，阈值由 Youden J 决定，没有纳入漏报成本、误报成本、资源约束或临床净获益。第七，Brier 分数虽优于常数概率基线，但尚未生成校准曲线、校准截距/斜率和分层校准结果。第八，原始 Challenge 数据并未排除所有 DNR/CMO 患者，预测可能受到治疗限制与临床决策影响；模型只能描述现有照护过程下的结局风险，不能解释治疗效果。
