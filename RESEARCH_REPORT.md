...
第一，死亡预测当前仍基于单次固定内部划分，主要指标没有 Bootstrap 或交叉验证置信区间；住院时长虽已为最终模型提供 Bootstrap 区间，也仍缺少时间外和医院外确认。模型和时间窗之间的小幅差异可能包含抽样波动。第二，本地数据没有医院、日期和患者身份，无法验证跨医院泛化、随时间漂移或同一患者重复住院造成的相关性。第三，缺失和测量频率含有临床流程信息，模型可能学习检查习惯；这种信息在同一数据环境下有预测价值，却可能在新医院失效。

第四，当前只处理了明确的 `-1` 缺失和重复记录，没有完成逐变量、经医学审核的异常值规则。第五，SAPS-I/SOFA 参照不是严格时间匹配，不能作为完全公平的早期评分对照。第六，阈值由 Youden J 决定，没有纳入漏报成本、误报成本、资源约束或临床净获益。第七，Brier 分数虽优于常数概率基线，但尚未生成校准曲线、校准截距/斜率和分层校准结果。第八，住院时长模型预测的是右偏且受死亡竞争事件影响的总住院天数，个体误差仍大；聚类虽有较高重复抽样稳定性，簇间分离度却较低，尚无临床专家或外部队列确认。第九，原始 Challenge 数据并未排除所有 DNR/CMO 患者，预测可能受到治疗限制与临床决策影响；模型只能描述现有照护过程下的结局风险，不能解释治疗效果。

## 十四、院内死亡二分类的下一阶段实验方案

下一阶段采用“固定问题、固定划分、逐项改变”的设计。原实验的训练集 8,400 例用于开发阶段的分层五折交叉验证，验证集 1,800 例用于候选方案确认和阈值选择，测试集 1,800 例只在方案冻结后评价。因为基线测试结果已经查看过，这个测试集已经不是研究全过程中从未见过的理想盲测集；后续将明确称为“基线后重新锁定的确认集”，不夸大其独立性。

填补实验先比较四条路线：中位数填补但不加缺失指示；当前的中位数加缺失指示；均值加缺失指示；以及梯度提升直接保留原生缺失值。测量次数和距末次测量时间会作为单独的缺失机制特征做消融，从而区分性能来自实际数值、缺失本身，还是临床复查频率。KNN 或迭代插补计算代价较高，也更容易生成临床上不合理的组合，只有在简单方案无法稳定时才进入第二轮候选。

清洗实验比较三种预先定义的方案：只处理 `-1`、空字段和同分钟重复值的原始基线；只使用训练集估计的 0.5%—99.5% 分位数对连续特征做 Winsorization；把明显违反单位或宽松生理边界的值先改为缺失，再进行相同填补。第三种方案的边界用于识别数据错误而非定义“正常人”，并将完整记录每个变量被置为缺失的数量。还会组合“宽松边界清洗+训练集分位数截断”，检查两种处理是否叠加获益。

为了控制计算量和多重比较，第一阶段只在临床提前量与数据完整度折中的 24 小时窗口上筛选，并让逻辑回归和梯度提升使用相同五折索引。主要选择指标为交叉验证 AUPRC，Brier 和 AUROC 为共同约束；如果 AUPRC 提高但 Brier 明显变差，不把它直接选为最优。第二阶段只让排名靠前的两种预处理方案进入 6、12、24、48 小时验证集比较。第三阶段冻结每个模型的一种方案和一个阈值，再评价确认集，并以患者记录级 Bootstrap 给出 95% 置信区间。

标签质量作为独立敏感性轴处理。主分析继续使用全部官方 `In-hospital_death` 标签，以便与基线和原数据定义比较；敏感性分析排除 95 条硬规则异常重新训练，并分别报告结局日期差值不超过 2、5、10 天人群中的结果。大差值病例不会在主分析中自动删除，因为这会选择性去除死亡病例并改变目标人群。

最终选出的处理方案还需绘制校准曲线，报告校准截距与斜率，并按 ICU 类型、性别和年龄组检查 AUROC、AUPRC、Brier、灵敏度与特异度。只有当性能增益在交叉验证、验证集、确认集和主要亚组中方向一致，且不是通过异常删除或缺失模式捷径获得，才认为清洗或填补策略带来较可信的改善。
