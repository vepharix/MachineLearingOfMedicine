...
这部分只扩展最终的 48 小时梯度提升模型，不重复前面的预处理筛选、消融、亚组和标签实验。所有结果都在实验 03 的固定划分和已选梯度提升管线下重新计算。在验证集上拟合 Platt 校准后，确认集校准截距由 0.209 接近至 0.045，斜率由 1.106 接近至 1.030，Brier 仅由 0.08423 降至 0.08410；isotonic 没有带来进一步收益。由于校准器仍只来自同一内部验证集，这个结果说明线性重校准可能有用，但不能替代在新医院重新估计概率。
